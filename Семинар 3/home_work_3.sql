...
SELECT salary AS 'Максимальные заработные платы'
FROM staff
ORDER BY salary DESC
LIMIT 5;

-- 3. Посчитайте суммарную зарплату (salary) по каждой специальности (роst)
SELECT SUM(salary) AS 'Суммарна заработная плата',
post AS 'Специальность'
FROM staff
GROUP BY post;

-- 4. Найдите кол-во сотрудников с специальностью (post) «Рабочий» в возрасте от 24 до 49 лет включительно.
SELECT COUNT(post) AS 'Количество рабочих возрастом от 24 до 49 лет включительно'
FROM staff
...
SELECT post AS 'Cпециальности cотрудников возрастом до 30 лет (включительно)'
FROM staff
GROUP BY post
HAVING AVG(age) <= 30;
